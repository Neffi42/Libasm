...
static void test_ft_strcpy(int *ret_value) {
  ...
  for (int i = 0; str[i] != NULL; i++) {
    ...
    print_result(str_cpy_ptr_2 == str_cpy_2 &&
                     memcmp(str_cpy_1, str_cpy_2, 11 * sizeof(char)) == 0,
                 ret_value);
    (void)ret_write;
  }
}
  ...
  for (int i = 0; content[i] != NULL; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        if (ret_2 == -1)
          errno_2 = errno;
        printf("ret: %zd == %zd, buf: \"%s\" == \"%s\", errno: %d == %d", ret_1,
               ret_2, buf_1, buf_2, errno_1, errno_2);
        print_result(ret_1 == ret_2 && strcmp(buf_1, buf_2) == 0 &&
                         errno_1 == errno_2,
                     ret_value);
      }
      close(fd_1);
